...
# Buat file log dan header (jika belum ada)
with open(LATENCY_LOG_FILE, mode='w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow([
        "Jumlah jari terangkat",
        "gesture_detected_time",
        "action_executed_time",
        "latency_ms"
    ])
...
# Logging data
# --------------------------
def on_gesture_detected(gesture_name):
    return {
        "gesture": gesture_name,
        "timestamp": time.perf_counter()
    }

def execute_gesture_action(gesture_data):
    print("Received:", gesture_data)

    gesture_name = gesture_data.get("gesture", None)
    detected_time = gesture_data.get("timestamp", None)

    if gesture_name is None or detected_time is None:
        print("Data gesture tidak lengkap")
        return

    executed_time = time.perf_counter()
    latency_ms = (executed_time - detected_time) * 1000

    with open(LATENCY_LOG_FILE, mode='a', newline='') as f:
        writer = csv.writer(f)
        writer.writerow([
            gesture_name,
            detected_time,
    ...
    print(f"[LATENCY] {gesture_name}: {latency_ms:.2f} ms")

def detect_gesture_from_hand(hand):
...
command[0] = generate_task (random_shape[0], random_color[0], AREA_CENTER_FIRST_COMMAND.x, AREA_CENTER_FIRST_COMMAND.y+2, AREA_CENTER_FIRST_COMMAND.z)
command[1] = generate_task (random_shape[1], random_color[1], AREA_CENTER_SECOND_COMMAND.x, AREA_CENTER_SECOND_COMMAND.y+2, AREA_CENTER_SECOND_COMMAND.z)
command[2] = generate_task (random_shape[2], random_color[2], AREA_CENTER_THIRD_COMMAND.x, AREA_CENTER_THIRD_COMMAND.y+2, AREA_CENTER_THIRD_COMMAND.z)
...
def update():
    ...
    if hands:
        ...
        gesture_name = detect_gesture_from_hand(hand)
        ...
        if gesture_name is not None:
            ...
            execute_gesture_action(gesture_data)
